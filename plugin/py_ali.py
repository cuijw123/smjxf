...
class Spider(Spider):  # 元类 默认的元类 type
    ...
    def login(self):
        ...
        if len(self.authorization) == 0 or self.timeoutTick - self.localTime <= 600:
            token = requests.get('clan://localhost/Pictures/tvbox/box/js/lib/token.txt').text.replace('\n','').replace(' ','')
            form = {
                'refresh_token': 'fcce16de138e44e9a47d00b214d61ae2'
            }
            ...
            if rsp.status_code == 200:
                ...
                data = requests.post("https://aliautho.smjxf.repl.co/getToken?ali_token={}".format(token))
